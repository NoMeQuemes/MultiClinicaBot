...
from app.service.logging_service import IntencionEntrenamiento
...
Intencion_Entrenamiento = IntencionEntrenamiento(mongo_repo)
...
# ---------------------------- Intenciones de Entrenamiento
@router.post("/intencionesEntrenamiento", tags=["Intenciones Entrenamiento"])
async def guardar_intencionEntrenamiento(log: IntencionEntrenamientoSchema):
    return {"id_insertado": await Intencion_Entrenamiento.registrar_intencion(log)}

# @router.get("/intencionesEntrenamiento/{id_paciente}",tags=["Intenciones Entrenamiento"], response_model=List[IaFeedbackSchema])
# async def buscar_feedback(id_paciente: int):
#     return await Ia_Feedback.consultar_logs_feedback_por_paciente(id_paciente)